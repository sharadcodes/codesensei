...
A VS Code extension that helps you understand a codebase, then tests your knowledge through a live, voice-driven **Ask Me Anything** session.
...
2. Open the workspace you want to understand.
3. Open the **Codebase Tutor** view in the Activity Bar.
4. Click the refresh icon next to **ACP Agents** to discover agents.
5. Click the check icon on an agent to select it.
6. Run **Codebase Tutor: Start Ask Me Anything** from the Command Palette.
7. Allow the agent to analyze the codebase, then answer the evaluator's questions.

## Configuration

Settings currently remain under the legacy `interviewLele.*` namespace so existing installations keep their configuration. Highlights:
...
| `realtime.instructions` | System prompt for the Ask Me Anything evaluator. |
...
| `acp.contextPrompt` | Prompt used to extract learning and knowledge-check context. |
